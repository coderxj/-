...
public class DoubleLinkList {
	...
	//复制表
	public DoubleLinkList copyList() {	//不能直接用clone方法，必须要一个个节点复制
		DoubleLinkList list = new DoubleLinkList();
		Node cur = head.next;
		for (int i = 0; i < iCount; i++,cur = cur.next) {
			Node temp = (Node)cur.clone();  //节点复制,不要直接赋值引用
			list.addFromTail(temp);   //因为加了个尾指针，现在头添加和尾添加时间复杂度都是0(1)
		}
		return list;
	}
	...
	public void mergeList(DoubleLinkList sl) {
		if (isEmpty())  //如果目的表为空
	...
	//合并链表(有序)
	public void mergeListOrder(DoubleLinkList sl,boolean bAsc) {
		mergeList(sl);
		selectSort(bAsc);
	}

	//合并链表(无序)，不会改变原表
	void mergeListNoChange(DoubleLinkList sl) {
		DoubleLinkList list = new DoubleLinkList();
	...
	//合并链表(有序),不会改变原表
	void mergeListNoChangeOrder(DoubleLinkList sl, boolean bAsc) {
		DoubleLinkList list = new DoubleLinkList();
		list = sl.copyList();           //复制链表,这样就不会改变原表
...
class Node implements Cloneable{  //实现该接口后，可进行对象的复制
	...
	public int data;
	public Node pre = null;   //前驱
	public Node next = null;  //后继
	Node(int d) {
		data = d;
	} 
