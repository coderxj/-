...
//重新分配空间（成倍增长）
template<typename T>
void SequenceList<T>::ReAlloc()
{
	int maxSize = m_maxSize;  //用来下面的复制数据用
	T* buf = new T[maxSize];  //用来暂时存放原来的数据
	for (int i = 0;i < maxSize;i++) //暂存原来的数据
		buf[i] = m_data[i];
	delete[] m_data;          //先释放原来的空间
	m_maxSize *= 2;			  //空间增大一倍
	m_data = new T[m_maxSize];//重新分配空间
	memset(m_data,0, m_maxSize);  //初始化
